...
/*				Core 0 : Turns on DS4										 */
/*				NASA JPL:  Change the memory test from writing the the exact name of the physical address in the physical address and reading from it to checkerboard */
...
#define start_physical_address 0x40000000
#define end_physical_address 0x40001FFF
...
int main(void)
{
	...
	//Writes the name of the actual physical addresses in the same physical addresses while reading it.
	//It goes through this process for a block in RAM or can be made to go through the whole RAM
	for(target =start_physical_address; target <= end_physical_address; target+=1){
	    writeval = 0xA5A5A5A5;
