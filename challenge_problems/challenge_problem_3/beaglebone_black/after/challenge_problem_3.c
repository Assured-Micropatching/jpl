...
#define BEAGLEBONE_BLACK
	...
	int main(int argc, char **argv) {
			...
			for(target = start_physicaladdress; target <= end_physicaladdress; target+=1){
				...
				*((unsigned long *) virt_addr) = writeval;
				...
				if(read_result != writeval){
					printf("physical address 0x%x; read physical addres 0x%X; write physical address 0x%X \n",target, read_result, writeval);
				}
